...
void rasterUpTriangle(const Triangle &t, unsigned char *buffer){
    ...
    //row_min = (int) fmax(0.0, row_min);   // Don't Draw Below Screen

    //Avoids divide by zero
    double left_slope = (t.X[0] - t.X[2]) / (t.Y[0] - t.Y[2]);
    double right_slope = (t.X[1] - t.X[0]) / (t.Y[1] - t.Y[0]);
    
    double scan_XL = t.X[0];
    double scan_XR = t.X[0];

    double burn;
    double bump = fabs(modf(t.Y[0], &burn));
    //double bump = fabs(t.Y[0] - trunc(t.Y[0]));
    scan_XL -= bump * left_slope;
    scan_XR -= bump * right_slope;
...
void rasterDownTriangle(const Triangle &t, unsigned char *buffer) {
    ...
    //row_max = (int) fmin(Y_SCALE - 1, row_max);   // Don't Draw Above Screen
    ...
    double burn, bump;
    //double bump = 1.0 - fabs(modf(t.Y[2], &burn));
    if (t.Y[2] < 0.0) {
        bump = (fabs(modf(t.Y[2], &burn)));
    }
    else {
        bump = (1.0 - (modf(t.Y[2], &burn)));
    }
    //double bump = fabs(t.Y[0] - trunc(t.Y[0]));
    scan_XL += bump * left_slope;
    scan_XR += bump * right_slope;
    ...
    for (int i = row_min; i <= row_max; i++) {
        ...
        // Bound Checking left right
        scan_XR = i / r_m - r_b / r_m;
        scan_XL = i / l_m - l_b / r_m;
...
void rasterTriangle(Triangle &t, unsigned char *buffer) {
    ...
    if ((eq(t.X[0], t.X[1]) && eq(t.Y[0], t.Y[1])) ||
        ...
        //cerr << "Degenerate Triangle is line or point" << endl;
        //t.print();
        //cerr << "Ignoring..." << endl;
        return;
    }
